...
export const toWords0 = (bytes: ArrayLike<number>) => {
  return convert(bytes, 8, 5, true);
};

export const encode0=(prefix: string, words: ArrayLike<number>, LIMIT?: number) =>{
  LIMIT = LIMIT || 90;
...
export const fromWords0 = (words: ArrayLike<number>) => {
  const res = convert(words, 5, 8, false);
  if (Array.isArray(res)) return res;
